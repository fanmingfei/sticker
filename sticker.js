...
editUtil.getCursortPosition = function(ctrl) { //获取光标位置函数
...
editUtil.insertTextAtArea = function(elem, value) {
    ...
    pos = editUtil.getCursortPosition(elem);
    s = elem.value;
...
Sticker.prototype.removeHsntEmoji = function() {
    ...
    if (_this.textType == 1) {
        ...
        var pos = editUtil.getCursortPosition(_this.textDom);
        if (newValue.has) {
...
Sticker.prototype.startCheck = function() {
    var _this = this;
    _this.textDom.addEventListener('focus', function() {
        _this.checkTimer = setInterval(function() {
            _this.removeHsntEmoji();
            // _this.parseAgentToImage();
        }, 200);
    });

    _this.textDom.addEventListener('blur', function() {
